...
const HELP = [
  ...
  "/stopall — stop EVERYTHING now: turns, subagents, jobs, browsers (owner)",
  "/pause /resume — freeze everything at the next step / continue (owner)",
  "/log — recent actions · /jobs — scheduled jobs · /cancel_job <id> (owner)",
  "/stop — stop the current reply (or the operator when idle)",
