...
// Vision capability now lives in @ares/core so the ENGINE can consult it at
// serialization time, not just the daemon at escalation time. Imported (not
// just re-exported) because this module calls it directly below; re-exported
// because every existing caller imports it from sessionFactory.
import { modelLikelyHasVision } from "@ares/core";
export { modelLikelyHasVision };
...
export async function createSessionWithSelection(
  ...
  if (resumeSessionId) {
    ...
    const session = new Session({
      ...
      requireVerificationEvidence: process.env.ARES_CODING_PROOF_GATE === "1",
