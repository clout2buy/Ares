...
export function modelLikelyHasVision(modelId: string): boolean {
  ...
  // pasted image to deepseek-flash was silently stripped.
  //
  // EXACTLY that id, though. The first fix spelled this `deepseek.*flash`,
  // which also claims vision for `deepseek-v4-flash:0731` — a DIFFERENT and
  // older model served by Ollama Cloud, and one this table has no evidence
  // about. Unknown must stay blind: the cost of guessing wrong here is a
  // silently stripped screenshot or a wasted turn, which is the bug this
  // whole file exists to prevent.
  if (/(?:^|[^a-z0-9])deepseek-flash(?:[^a-z0-9]|$)/.test(id)) return true;
