...
function App() {
  ...
  return (
    ...
    >
      <ThreeScene running={running} theme={theme} />
...
type SceneMode = "signal" | "matrix" | "storm";
...
function scenePalette(mode: SceneMode): ScenePalette {
  if (mode === "matrix") {
    return { key: 0xbb66ff, rim: 0x00ffbb, fog: 0x12061f, glass: 0xb67cff, emissive: 0x5a0088, accent: 0x00ffbb, core: 0x9bffe6, dust: 0x9b7bff };
  }
  if (mode === "storm") {
    return { key: 0x99c9ff, rim: 0xaed8ff, fog: 0x06101f, glass: 0xcfe6ff, emissive: 0x1c74ff, accent: 0xf2f8ff, core: 0xeaf4ff, dust: 0xaeccff };
  }
  return { key: 0xffffff, rim: 0x5ea8ff, fog: 0x0a1626, glass: 0xbfe0ff, emissive: 0x16407a, accent: 0xffb27a, core: 0xeafff7, dust: 0x8fd0ff };
}

function ThreeScene({ running, theme }: { running: boolean; theme: ThemeName }) {
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    const mode: SceneMode = theme === "matrix" ? "matrix" : theme === "storm" ? "storm" : "signal";
    ...
    scene.fog = new THREE.FogExp2(palette.fog, 0.034);
    const camera = new THREE.PerspectiveCamera(42, 1, 0.1, 100);
    camera.position.set(0, 0, 9);

    scene.add(new THREE.AmbientLight(0xffffff, 0.5));
    const key = new THREE.PointLight(palette.key, 5.4, 34);
    key.position.set(5, 5, 6);
    const rim = new THREE.PointLight(palette.rim, 3.1, 34);
    rim.position.set(-6, -3, 2);
    scene.add(key, rim);

    const root = new THREE.Group();
    scene.add(root);
    buildSceneObject(root, mode, palette);
    ...
    const animate = () => {
      ...
      root.rotation.y += running ? 0.006 : 0.003;
...
function buildSceneObject(root: THREE.Group, mode: SceneMode, palette: ScenePalette) {
  if (mode === "matrix") {
    ...
    const eye = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: palette.core, emissive: palette.accent, emissiveIntensity: 1.7, roughness: 0.2 }),
    );
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(1.85, 0.03, 18, 160),
      new THREE.MeshStandardMaterial({ color: palette.accent, emissive: palette.accent, emissiveIntensity: 0.85, metalness: 0.4, roughness: 0.3, transparent: true, opacity: 0.85 }),
    );
    ring.rotation.x = Math.PI / 2.4;
    root.add(crystal, eye, ring);
    return;
  }
  ...
  if (mode === "storm") {
    ...
    const halo = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: palette.accent, transparent: true, opacity: 0.06, blending: THREE.AdditiveBlending, depthWrite: false }),
    );
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(2.55, 0.024, 18, 180),
      new THREE.MeshStandardMaterial({ color: palette.accent, emissive: palette.accent, emissiveIntensity: 0.8, transparent: true, opacity: 0.7 }),
    );
    ring.rotation.x = Math.PI / 2.5;
    root.add(bolt, halo, ring);
    return;
  }
  ...
  const orbit = new THREE.Mesh(
    new THREE.TorusGeometry(2.95, 0.04, 18, 200),
    new THREE.MeshStandardMaterial({ color: palette.accent, emissive: palette.accent, emissiveIntensity: 0.9, metalness: 0.5, roughness: 0.3, transparent: true, opacity: 0.9 }),
  );
  orbit.rotation.x = Math.PI / 2.6;
  const core = new THREE.Mesh(
    new THREE.SphereGeometry(0.5, 36, 24),
    new THREE.MeshStandardMaterial({ color: palette.core, emissive: palette.rim, emissiveIntensity: 1.5, roughness: 0.2 }),
  );
  root.add(glass, orbit, core);
}
