...
test("intent governor: architecture questions recall without becoming housekeeping", () => {
  const intent = classifyUserIntent("what do u think about Ares memory and operator architecture?");
  ...
  const reminder = buildForegroundReminder("what do u think about Ares memory and operator architecture?");
...
test("intent governor: long autonomous upgrades are foreground missions", () => {
  const intent = classifyUserIntent("go all out and make Ares the best no matter how long it takes");
